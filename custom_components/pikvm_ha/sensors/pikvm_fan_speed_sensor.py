...
class PiKVMFanSpeedSensor(PiKVMBaseSensor):
    ...
    def state(self):
        ...
        if (
            ...
            or not self.coordinator.data.get("fan")
            or not self.coordinator.data.get("fan", {}).get("state")
        ):
            return None

        if self.hall_available:
            hall_data = get_nested_value(
                self.coordinator.data, ["fan", "state", "hall"]
            )
    ...
    def extra_state_attributes(self):
        ...
        if (
            ...
            and self.coordinator.data.get("fan")
            and self.coordinator.data.get("fan", {}).get("state")
        ):
